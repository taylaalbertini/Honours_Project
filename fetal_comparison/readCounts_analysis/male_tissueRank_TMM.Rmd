...
# # set path for male TMM normalised ncpm counts
import_mean_male_cpms_TMM <- file.path(projectDir, "mean_male_cpms_TMM.csv")
```

# import data tables 
```{r}
# import mean male ncpms table
ncpms_male_TMM <- read_delim(file = import_mean_male_cpms_TMM,
                                   col_names = TRUE,
                                   delim = ",") %>%
  as.data.frame() %>%
...
nCPMs_rank_male <- dplyr::left_join(ranked_male_TMM, nCPMs_male_TMM, copy = TRUE, by = join_by(geneid))
  
# select for when placenta_rank = 6 (placenta ncpm is ranked last for the given genes)
placenta_male_6_TMM <- subset(nCPMs_rank_male, male_placenta_rank == "6")
```

# filter for when nCPM > 1 for tissue rank 5
```{r}
# filter for where rank = 5 and cpm > 1
nCPM_rank_5_TMM <- dplyr::filter(placenta_male_6_TMM, male_brain_rank == "5" & male_brain_ncpm > 1 | male_heart_rank == "5" & male_heart_ncpm > 1 | male_liver_rank == "5" & male_liver_ncpm > 1 | male_lung_rank == "5" & male_lung_ncpm > 1 | male_muscle_rank == "5" & male_muscle_ncpm > 1)
```

# separate nCPM_rank_5_TMM into rank and ncpm tables respectively  
```{r}
...
## convert 'geneids' from row names into a column
...
# convert nCPM_5_TMM into long form
## convert 'geneids' from row names into a column
...
rank_placenta_long <- dplyr::filter(rank_5_TMM_long, rank == "6")
...
# join placenta_6_nCPM and nCPM_rank_5_TMM_long creating a df with both the nCPMs of rank 5 and 6
  nCPM5_nCPM6_TMM_male <- dplyr::left_join(nCPM_rank_5_TMM_long, placenta_6_nCPM, by = "geneid")
...
placenta_m_depleted_TMM <- nCPM5_nCPM6_TMM_male[nCPM5_nCPM6_TMM_male$nCPM_ratio >3,]
```

# tidy and export placenta_m_depleted_TMM
```{r}
...
# export placenta_m_depleted_TMM into project rawData for further analysis  
