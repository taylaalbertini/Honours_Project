...
colnames(TMM_depletedF) <- gsub("nCPM_6", "nCPM_placenta_TMM", colnames(TMM_depletedF))
...
colnames(DESq_depletedF) <- gsub("nCPM_6", "nCPM_placenta_DESq", colnames(DESq_depletedF))
...
## export for plots
write.csv(placentaF_depleted, file = file.path(projectDir, "placentaF_depleted.csv"))

# left join genecode and placentaF_depleted to add gene signatures 
...
```{r}
placentaF_ordered <- placentaF_depleted_annotated[order(placentaF_depleted_annotated$nCPM_ratio_TMM, decreasing = TRUE),]
```

# export placentaF_depleted_annotated_2
```{r}
write.csv(placentaF_depleted_annotated_2, file = file.path(projectDir, "placentaF_depleted_annotated.csv"))
```
