---
title: "DESeq_brainM_rankAnalysis"
...
brain_male_6_DESq <- subset(nCPMs_rank_male_DESq, male_brain_rank == "6")
```

# filter for when nCPM > 1 for tissue rank 5
```{r}
# filter for where rank = 5 and ncpm > 1
nCPM_rank_5_DESq <- dplyr::filter(brain_male_6_DESq, male_placenta_rank == "5" & male_placenta_ncpm > 1 | male_heart_rank == "5" & male_heart_ncpm > 1 | male_liver_rank == "5" & male_liver_ncpm > 1 | male_lung_rank == "5" & male_lung_ncpm > 1 | male_muscle_rank == "5" & male_muscle_ncpm > 1)
```
...
nCPM5_nCPM6_DESq_male <- dplyr::left_join(nCPM_rank_5_DESq_long, brain_6_nCPM, by = "geneid")
...
brain_m_depleted_DESq <- nCPM5_nCPM6_DESq_male[nCPM5_nCPM6_DESq_male$nCPM_ratio >3,]
```

# tidy and export brain_m_depleted_DESq
```{r}
# replace column name 'geneid' with 'ensembl'
colnames(brain_m_depleted_DESq) <- gsub("geneid", "ensembl", colnames(brain_m_depleted_DESq))

# export brain_m_depleted_DESq in project rawData for further analysis 
write.csv(brain_m_depleted_DESq, file = file.path(projectDir, "DESq_brainM_depleted.csv"))
```
