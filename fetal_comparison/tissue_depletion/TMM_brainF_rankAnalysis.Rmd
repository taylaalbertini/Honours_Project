...
brain_female_6_TMM <- subset(nCPMs_rank_female_TMM, female_brain_rank == "6")
```

# filter for when nCPM > 1 for tissue rank 5
```{r}
# filter for where rank = 5 and cpm > 1
nCPM_rank_5_TMM <- dplyr::filter(brain_female_6_TMM, female_placenta_rank == "5" & female_placenta_ncpm > 1 | female_heart_rank == "5" & female_heart_ncpm > 1 | female_liver_rank == "5" & female_liver_ncpm > 1 | female_lung_rank == "5" & female_lung_ncpm > 1 | female_muscle_rank == "5" & female_muscle_ncpm > 1)
```
...
nCPM5_nCPM6_TMM_female <- dplyr::left_join(nCPM_rank_5_TMM_long, brain_6_nCPM, by = "geneid")
...
brain_f_depleted_TMM <- nCPM5_nCPM6_TMM_female[nCPM5_nCPM6_TMM_female$nCPM_ratio >3,]
```

# tidy and export brain_f_depleted_TMM
```{r}
# replace column name 'geneid' with 'ensembl'
colnames(brain_f_depleted_TMM) <- gsub("geneid", "ensembl", colnames(brain_f_depleted_TMM))

# export brain_f_depleted_TMM into project rawData for further analysis 
write.csv(brain_f_depleted_TMM, file = file.path(projectDir, "TMM_brainF_depleted.csv"))
```
