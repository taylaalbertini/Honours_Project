---
title: "DESeq_heartF_rankAnalysis"
...
# ranking DESq normalised ncpms by female_tissue
```{r}
...
ranked_female_cpms_DESq <- data.frame(ncpms_female_DESq) %>%
...
heart_female_6_DESq <- subset(nCPMs_rank_female_DESq, female_heart_rank == "6")
```

# filter for when nCPM > 1 for tissue rank 5
```{r}
# filter for where rank = 5 and cpm > 1
nCPM_rank_5_DESq <- dplyr::filter(heart_female_6_DESq, female_placenta_rank == "5" & female_placenta_ncpm > 1 | female_brain_rank == "5" & female_brain_ncpm > 1 | female_liver_rank == "5" & female_liver_ncpm > 1 | female_lung_rank == "5" & female_lung_ncpm > 1 | female_muscle_rank == "5" & female_muscle_ncpm > 1)
```
...
# convert cpm_5_DESq into long form
...
rank_heart_long <- filter(rank_5_DESq_long, rank == "6")

# join nCPM_5_DESq_long to rank_heart_long to add ncpms for geneids where placenta rank = 6
...
nCPM5_nCPM6_DESq_female <- dplyr::left_join(nCPM_rank_5_DESq_long, heart_6_nCPM, by = "geneid")
...
heart_f_depleted_DESq <- nCPM5_nCPM6_DESq_female[nCPM5_nCPM6_DESq_female$nCPM_ratio >3,]
```

# tidy and export heart_f_depleted_DESq
```{r}
# replace column name 'geneid' with 'ensembl'
colnames(heart_f_depleted_DESq) <- gsub("geneid", "ensembl", colnames(heart_f_depleted_DESq))

# export heart_f_depleted_DESq into project rawData for further analysis 
write.csv(heart_f_depleted_DESq, file = file.path(projectDir, "DESq_heartF_depleted.csv"))
```
