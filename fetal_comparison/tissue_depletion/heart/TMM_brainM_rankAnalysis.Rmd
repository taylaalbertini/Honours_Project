...
heart_male_6_TMM <- subset(nCPMs_rank_male_TMM, male_heart_rank == "6")
```

# filter for when nCPM > 1 for tissue rank 5
```{r}
# filter for where rank = 5 and cpm > 1
nCPM_rank_5_TMM <- dplyr::filter(heart_male_6_TMM, male_placenta_rank == "5" & male_placenta_ncpm > 1 | male_brain_rank == "5" & male_brain_ncpm > 1 | male_lung_rank == "5" & male_lung_ncpm > 1 | male_muscle_rank == "5" & male_muscle_ncpm > 1 | male_liver_rank == "5" & male_liver_ncpm > 1)
```
...
# join nCPM_5_TMM_long to rank_heart_long to add ncpms for geneids where placenta rank = 6
heart_6_nCPM <- dplyr::left_join(rank_heart_long, nCPM_5_TMM_long, by = c("geneid", "tissue"))
...
nCPM5_nCPM6_TMM_male <- dplyr::left_join(nCPM_rank_5_TMM_long, heart_6_nCPM, by = "geneid")
...
heart_m_depleted_TMM <- nCPM5_nCPM6_TMM_male[nCPM5_nCPM6_TMM_male$nCPM_ratio >3,]
```

# tidy and export heart_m_depleted_TMM
```{r}
# replace column name 'geneid' with 'ensembl'
colnames(heart_m_depleted_TMM) <- gsub("geneid", "ensembl", colnames(heart_m_depleted_TMM))

# export heart_m_depleted_TMM into project rawData for further analysis 
write.csv(heart_m_depleted_TMM, file = file.path(projectDir, "TMM_heartM_depleted.csv"))
```
