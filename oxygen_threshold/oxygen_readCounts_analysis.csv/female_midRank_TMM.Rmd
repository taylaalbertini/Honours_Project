...
import_mean_female_cpms_TMM <- file.path(projectDir, "mean_female_cpms_oxy_TMM.csv")
```
...
# isolate all instances where mid placenta is ranked 2nd
```{r}
...
ranked_female_TMM <- t(ranked_female_cpms_TMM) %>%
  ...
  colnames(ranked_female_TMM) <- gsub("female_placenta_early", "female_placenta_early_rank", colnames(ranked_female_TMM))
...
nCPMs_female_TMM <- data.frame(mean_female_cpms_TMM_cp) %>%
 ...
 colnames(nCPMs_female_TMM) <- gsub("female_placenta_early", "female_placenta_early_cpm", colnames(nCPMs_female_TMM))
...
# select for when mid_placenta_rank = 2
placenta_female_2_TMM <- subset(nCPMs_rank_female, female_placenta_mid_rank == "2")
```

# nCPM > 1 for tissue rank 1 and convert tables into one long form table
```{r}
# filter for where rank = 1 and cpm > 1
nCPM_rank_1_TMM <- dplyr::filter(placenta_female_2_TMM, female_placenta_early_rank == "1" & female_placenta_early_cpm > 1)
...
nCPM_1_TMM_long <- tidyr::gather(nCPM_1_TMM_long, tissue, cpm, female_placenta_early, female_placenta_mid)
...
## rename, remove "_cpm"
 names(rank_1_TMM_long) <- gsub("_rank", "", names(rank_1_TMM_long))
  #convert into long form
rank_1_TMM_long <- tidyr::gather(rank_1_TMM_long, tissue, rank, female_placenta_early, female_placenta_mid)
rank_1_TMM_long_2 <- dplyr::filter(rank_1_TMM_long, rank == "1")

# join the long form tables - nCPM_1_TMM_long and rank_2_TMM
nCPM_rank_1_TMM_long <- dplyr::left_join(rank_1_TMM_long_2, nCPM_1_TMM_long, by = c("geneid", "tissue"))
## rename cpm to nCPM_1
...
```{r}
write.csv(placenta_f_depleted_TMM, file = file.path(projectDir, "TMM_placentaF_mid_depleted.csv"))
```
