...
placenta_male_2_TMM <- subset(nCPMs_rank_male, male_placenta_mid_rank == "2")
```

# nCPM > 1 for tissue rank 1 and convert tables into one long form table
```{r}
# filter for where rank = 1 and cpm > 1
nCPM_rank_1_TMM <- dplyr::filter(placenta_male_2_TMM, male_placenta_early_rank == "1" & male_placenta_early_cpm > 1)
...
```{r}
write.csv(placenta_m_depleted_TMM, file = file.path(projectDir, "TMM_placentaM_mid_depleted.csv"))
```
