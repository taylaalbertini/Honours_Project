...
import_TMM_depletedF <- file.path(projectDir, "TMM_placentaF_early_depleted.csv")
import_DESq_depletedF <- file.path(projectDir, "DESq_placentaF_early_depleted.csv")
...
## filter the df to only including the relevant pieces of information and make specific to normalisation technique
    ...
    colnames(TMM_depletedF) <- gsub("nCPM_2", "nCPM_early_placenta_TMM", colnames(TMM_depletedF))
    ...
    colnames(DESq_depletedF) <- gsub("nCPM_2", "nCPM_early_placenta_DESq", colnames(DESq_depletedF))
...
## export for plots
write.csv(placentaF_depleted, file.path(projectDir, "earlyF_depleted.csv"))
...
```{r}
write.csv(placentaF_depleted_annotated_2, file = file.path(projectDir, "early_placentaF_depleted_annotated.csv"))
```
