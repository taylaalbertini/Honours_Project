...
import_mean_male_cpms_TMM <- file.path(projectDir, "mean_male_cpms_oxy_TMM.csv")
```

# import files
```{r}
# import mean female cpms table
mean_male_cpms_TMM <- read_delim(file = import_mean_male_cpms_TMM,
                                   col_names = TRUE,
                                   delim = ",")

# remove the first column - "...1"
mean_male_cpms_TMM <- mean_male_cpms_TMM[,-1]

# copy mean_cpms for subsequent analysis 
mean_male_cpms_TMM_cp <- as.data.frame(mean_male_cpms_TMM)
```
...
mean_male_cpms_TMM[, paste0(names(mean_male_cpms_TMM)[-1], "_Rank")] <- apply(mean_male_cpms_TMM [, -1, drop = FALSE], 2, calculate_rank)

## convert sex_tissue into row names
mean_male_cpms_TMM <- tibble::column_to_rownames(mean_male_cpms_TMM, var = "sex_tissue")
# create a df selecting just for _Rank
ranked_male_cpms_TMM <- data.frame(mean_male_cpms_TMM) %>%
...
ranked_male_TMM <- t(ranked_male_cpms_TMM) %>%
  ...
  colnames(ranked_male_TMM) <- gsub("male_placenta_early", "male_placenta_early_rank", colnames(ranked_male_TMM))
  colnames(ranked_male_TMM) <- gsub("male_placenta_mid", "male_placenta_mid_rank", colnames(ranked_male_TMM))
...
nCPMs_male_TMM <- data.frame(mean_male_cpms_TMM_cp) %>%
 ...
 colnames(nCPMs_male_TMM) <- gsub("male_placenta_early", "male_placenta_early_cpm", colnames(nCPMs_male_TMM))
  colnames(nCPMs_male_TMM) <- gsub("male_placenta_mid", "male_placenta_mid_cpm", colnames(nCPMs_male_TMM))

# combine via left_join nCPMs_male_TMM and ranked_female_TMM
...
placenta_male_2_TMM <- subset(nCPMs_rank_male, male_placenta_early_rank == "2")
```

# nCPM > 1 for tissue rank 1 and convert tables into one long form table
```{r}
# filter for where rank = 1 and cpm > 1
nCPM_rank_1_TMM <- dplyr::filter(placenta_male_2_TMM, male_placenta_mid_rank == "1" & male_placenta_mid_cpm > 1)
...
nCPM_1_TMM_long <- tidyr::gather(nCPM_1_TMM_long, tissue, cpm, male_placenta_early, male_placenta_mid)
...
## rename, remove "_cpm"
 names(rank_1_TMM_long) <- gsub("_rank", "", names(rank_1_TMM_long))
  #convert into long form
rank_1_TMM_long <- tidyr::gather(rank_1_TMM_long, tissue, rank, male_placenta_early, male_placenta_mid)
...
# tidy placenta_m_depleted_TMM table
```{r}
# replace gneeid with ensembl
colnames(placenta_m_depleted_TMM) <- gsub("geneid", "ensembl", colnames(placenta_m_depleted_TMM))
```

# export placenta_m_depleted_TMM
```{r}
write.csv(placenta_m_depleted_TMM, file = file.path(projectDir, "TMM_placentaM_early_depleted.csv"))
```
