...
import_mean_female_cpms_DESq <- file.path(projectDir, "mean_female_cpms_oxy_DESq.csv")
```
...
ranked_female_DESq <- t(ranked_female_cpms_DESq) %>%
  ...
  colnames(ranked_female_DESq) <- gsub("female_placenta_early", "female_placenta_early_rank", colnames(ranked_female_DESq))
...
nCPMs_female_DESq <- data.frame(mean_female_cpms_DESq_cp) %>%
 ...
 colnames(nCPMs_female_DESq) <- gsub("female_placenta_early", "female_placenta_early_cpm", colnames(nCPMs_female_DESq))
...
placenta_female_2_DESq <- subset(nCPMs_rank_female, female_placenta_mid_rank == "2")
```

# nCPM > 1 for tissue rank 1 and convert tables into one long form table
```{r}
# filter for where rank = 1 and cpm > 1
nCPM_rank_1_DESq <- dplyr::filter(placenta_female_2_DESq, female_placenta_early_rank == "1" & female_placenta_early_cpm > 1)
...
nCPM_1_DESq_long <- tidyr::gather(nCPM_1_DESq_long, tissue, cpm, female_placenta_early, female_placenta_mid)
...
## rename, remove "_cpm"
 names(rank_1_DESq_long) <- gsub("_rank", "", names(rank_1_DESq_long))
  #convert into long form
rank_1_DESq_long <- tidyr::gather(rank_1_DESq_long, tissue, rank, female_placenta_early, female_placenta_mid)
rank_1_DESq_long_2 <- dplyr::filter(rank_1_DESq_long, rank == "1")

# join the long form tables - nCPM_1_TMM_long and rank_1_TMM
...
```{r}
write.csv(placenta_f_depleted_DESq, file = file.path(projectDir, "DESq_placentaF_mid_depleted.csv"))
```
