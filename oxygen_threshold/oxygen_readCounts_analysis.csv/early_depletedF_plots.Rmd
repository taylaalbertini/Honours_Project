---
anatitle: "early_depletedF_plots"
...
path_save <- "/Users/taylaalbertini/Desktop/R studio/honours_project/oxygen_threshold/analysis/heatmaps/earlyF_depleted_heatmap.jpeg"
...
annot_depleted <- read_delim(file = file.path(projectDir, "early_placentaF_depleted_annotated.csv"),
                             col_names = TRUE,
                             delim = ",") %>%
  as.data.frame() %>%
  ## remove the first column "...1" that;s input when the file is imported
...
ggplot(data = scatter_plot,
  ...
  geom_point(#size = scatter_plot$effect_size * 0.05,
             color = "black",
             shape = 1) +
  #ggtitle("Depleted female early gestation placental transcripts compared with mid-gestation") +
  ...
  geom_text_repel(data = subset(scatter_plot, nCPM_ratio_TMM > 20), ## annotates/labels the ten-greatest nCPM ratio genes 
            aes(label = rownames(subset(scatter_plot, nCPM_ratio_TMM > 20))),
            nudge_y = 50,
            size          = 2.5,
            box.padding   = 1.5,
            point.padding = 0.5,
...
ggsave("earlyfemaleplot.png", width = 7, height = 7, units = "in") ## exported the last graph printed
```
