...
import_mean_male_cpms_DESq <- file.path(projectDir, "mean_male_cpms_oxy_DESq.csv")
```
...
placenta_male_2_DESq <- subset(nCPMs_rank_male, male_placenta_early_rank == "2")
```
...
# tidy placenta_m_depleted_DESq table
```{r}
# replace gneeid with ensembl
colnames(placenta_m_depleted_DESq) <- gsub("geneid", "ensembl", colnames(placenta_m_depleted_DESq))
```

# export placenta_m_depleted_DESq
```{r}
write.csv(placenta_m_depleted_DESq, file = file.path(projectDir, "DESq_placentaM_early_depleted.csv"))
```
