...
colnames(ncpms_male_2) <- gsub("X1", "Male Placenta (early-gestation)", colnames(ncpms_male_2))
...
colnames(heat_plot) <- gsub("nCPM_mid_placenta_TMM", "Male Placenta (mid-gestation)", colnames(heat_plot))
...
heat_plot_ordered$`Male Placenta (early-gestation)` <- as.numeric(heat_plot_ordered$`Male Placenta (early-gestation)`)
## change order so that placenta is last
heat_plot_ordered <- dplyr::relocate(heat_plot_ordered, `Male Placenta (mid-gestation)`, .after = `Male Placenta (early-gestation)`)
...
path_save <- "/Users/taylaalbertini/Desktop/R studio/honours_project/oxygen_threshold/analysis/heatmaps/midM_depleted_heatmap.jpeg"
