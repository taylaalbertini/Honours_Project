...
known_markers <- known_markers[rowSums(known_markers[,-1])>0,]
```
...
for (gene in genes_of_interest) {
  # Create the plot
  p <- ggplot(merged_data, aes(x = x, y = y)) +
...
# load in unfiltered reference data
```{r}
library(data.table)
# Read the file
...
# annotate the clusters based on un-adjusted reference
```{r}
# Function to annotate clusters
annotate_clusters <- function(markers, known_markers) {
  # Create an empty list to store annotations
