...
import_mean_male_cpms_TMM <- file.path(projectDir, "mean_male_cpms_termC_TMM.csv")
```
...
ranked_male_TMM <- t(ranked_male_cpms_TMM) %>%
  ...
  colnames(ranked_male_TMM) <- gsub("male_placenta_term", "male_placenta_term_rank", colnames(ranked_male_TMM))
...
nCPMs_male_TMM <- data.frame(mean_male_cpms_TMM_cp) %>%
 ...
 colnames(nCPMs_male_TMM) <- gsub("male_placenta_mid", "male_placenta_mid_cpm", colnames(nCPMs_male_TMM))
  colnames(nCPMs_male_TMM) <- gsub("male_placenta_term", "male_placenta_term_cpm", colnames(nCPMs_male_TMM))
...
placenta_male_2_TMM <- subset(nCPMs_rank_male, male_placenta_term_rank == "2")
```
...
nCPM_1_TMM_long <- tidyr::gather(nCPM_1_TMM_long, tissue, cpm, male_placenta_mid, male_placenta_term)
...
## rename, remove "_cpm"
 names(rank_1_TMM_long) <- gsub("_rank", "", names(rank_1_TMM_long))
  #convert into long form
rank_1_TMM_long <- tidyr::gather(rank_1_TMM_long, tissue, rank, male_placenta_mid, male_placenta_term)
...
```{r}
write.csv(placenta_m_depleted_TMM, file = file.path(projectDir, "TMM_placentaM_term_depleted.csv"))
```
