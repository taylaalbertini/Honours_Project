---
title: "female_placenta_rankAnalysis_DESq"
...
placenta_2_DESq <- subset(nCPMs_rank, female_placenta_rank == "2")
```

# nCPM > 1 for tissue rank 1 and convert tables into one long form table
```{r}
# filter for where rank = 1 and cpm > 1
nCPM_rank_1_DESq <- dplyr::filter(placenta_2_DESq, male_placenta_rank == "1" & male_placenta_cpm > 1)
...
placentaF_depleted_DESq <- nCPM1_nCPM2_DESq[nCPM1_nCPM2_DESq$nCPM_ratio >3,]
```

# tidy placenta_f_depleted_DESq table
```{r}
# replace gneeid with ensembl
colnames(placentaF_depleted_DESq) <- gsub("geneid", "ensembl", colnames(placentaF_depleted_DESq))
```

# export placentaF_depleted_DESq
```{r}
write.csv(placentaF_depleted_DESq, file = file.path(projectDir, "DESq_placentaF_depleted.csv"))
```
