...
## rename column names
colnames(ncpms_male_2) <- gsub("X1", "Female Placenta", colnames(ncpms_male_2))
...
colnames(heat_plot) <- gsub("nCPM_male_placenta_TMM", "Male Placenta", colnames(heat_plot))
...
heat_plot_ordered <- dplyr::relocate(heat_plot_ordered, `Male Placenta`, .after = `Female Placenta`)
...
path_save <- "/Users/taylaalbertini/Desktop/R studio/honours_project/sexSpecific_comparison/analysis/heatmaps/male_depleted_heatmap_update.jpeg"
...
annot_depleted <- read_delim(file = file.path(projectDir, "placenta_male_depleted_annotated.csv"),
                             col_names = TRUE,
                             delim = ",") %>%
  as.data.frame() %>%
  ## remove the first column "...1" that;s input when the file is imported
...
scatter <- dplyr::select(annot_depleted, hgnc_symbol, nCPM_male_placenta_TMM, nCPM_ratio_TMM)
...
ggplot(data = scatter_plot,
       aes(x = nCPM_male_placenta_TMM,
           y = nCPM_ratio_TMM)) +
  geom_point(#size = scatter_plot$effect_size * 0.05,
             color = "black",
             shape = 1) +
  #ggtitle("Depleted male mid-gestation placental transcripts compared with female mid-gestation placenta") +
  ...
  scale_y_continuous(breaks = c(pretty(range(scatter_plot$y)), 3, 40, 80, 120, 150)) +
...
ggsave("maleplotvsfemale.png", width = 7, height = 7, units = "in") ## exported the last graph printed
```
