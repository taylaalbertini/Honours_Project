---
title: "female_placenta_rankAnalysis_TMM"
...
placenta_2_TMM <- subset(nCPMs_rank, female_placenta_rank == "2")
```

# nCPM > 1 for tissue rank 1 and convert tables into one long form table
```{r}
# filter for where rank = 1 and cpm > 1
nCPM_rank_1_TMM <- dplyr::filter(placenta_2_TMM, male_placenta_rank == "1" & male_placenta_cpm > 1)
...
placentaF_depleted_TMM <- nCPM1_nCPM2_TMM[nCPM1_nCPM2_TMM$nCPM_ratio >3,]
```

# tidy placentaF_depleted_TMM table
```{r}
# replace gneeid with ensembl
colnames(placentaF_depleted_TMM) <- gsub("geneid", "ensembl", colnames(placentaF_depleted_TMM))
```

# export placentaF_depleted_TMM
```{r}
write.csv(placentaF_depleted_TMM, file = file.path(projectDir, "TMM_placentaF_depleted.csv"))
```
